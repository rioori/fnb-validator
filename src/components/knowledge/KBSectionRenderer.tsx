...
/** Parse [text](url) markdown links inline. AI engines + readers can verify sources. */
  ...
  while ((match = regex.exec(text)) !== null) {
    ...
    parts.push(
      <a
        ...
        className="text-cta underline decoration-dotted hover:decoration-solid"
      >
        {match[1]}
      </a>
    );
      ...
      return (
        <>
          ...
          <div className="text-[13px] text-text leading-relaxed my-2 clay-sm bg-mint-light px-4 py-3">
            {linkify(section.content as string)}
          </div>
        </>
      );
          ...
          <div className="space-y-1.5 my-2">
            {(section.content as KBTableRow[]).map((row, i) => (
              <div key={i} className="clay-sm p-3 flex items-center gap-3 text-[13px] max-md:flex-col max-md:items-start max-md:gap-1.5">
                ...
                <span className="text-text-muted text-[12px] flex-1">{linkify(row.note)}</span>
              </div>
            ))}
          </div>
        </>
          ...
          <div className="grid grid-cols-2 gap-2.5 my-2 max-[480px]:grid-cols-1">
            {(section.content as KBStat[]).map((stat, i) => (
              <div key={i} className="clay-sm p-4 text-center">
                ...
                <div className="text-[11px] text-text-muted mt-1">{linkify(stat.desc)}</div>
              </div>
            ))}
          </div>
        </>
              ...
              return (
                <div key={i} className="mb-3 last:mb-0 relative">
                  ...
                  <div className="clay-sm p-3">
                    ...
                    <div className="text-[12px] text-text-muted mt-1 leading-relaxed">{linkify(step.desc)}</div>
                  </div>
                </div>
              );
            })}
              ...
              return (
                <div key={i} className={`rounded-2xl border ${bg} p-3 flex gap-3`}>
                  ...
                  <div>
                    ...
                    <div className="text-[12px] text-text-muted mt-0.5 leading-relaxed">{linkify(item.desc)}</div>
                  </div>
                </div>
              );
            })}
