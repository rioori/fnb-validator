...
export default function StepInvestment() {
  ...
  const cats = isExisting
    ? [
        { name: t.wizard.stepInvestment.sunkCostLabel, val: store.sunkCost, color: 'var(--color-text)' },
  ...
  const overBudget = !isExisting && store.budget > 0 && totalInv > store.budget;

  return (
    <div>
      <h2 className="text-lg font-bold mb-1 text-text font-[family-name:var(--font-heading)]">
        {isExisting ? t.wizard.stepInvestment.titleExisting : t.wizard.stepInvestment.title}
      </h2>
      <p className="text-text-muted text-[13px] mb-3">
        {isExisting ? t.wizard.stepInvestment.descExisting : t.wizard.stepInvestment.desc}
      </p>

      {isExisting ? (
        <>
          {/* Existing mode: simplified — sunk cost + working cap */}
          <SectionCard title={t.wizard.stepInvestment.sunkCostLabel}>
            <VNDInput value={store.sunkCost} onChange={store.setSunkCost} />
            <p className="text-[11px] text-text-muted mt-1">{t.wizard.stepInvestment.sunkCostHint}</p>
          </SectionCard>
        ...
        <>
          {/* New mode: full investment breakdown */}
