...
export default function StepModel() {
  ...
  const handleNext = () => {
    if (!selectedModel) { alert('Chọn mô hình trước!'); return; }
    nextStep();
  };
  ...
  return (
    <div>
      <h2 className="text-lg font-bold mb-1 text-text font-[family-name:var(--font-heading)]">
        Bạn muốn mở loại hình nào?
      </h2>
      <p className="text-text-muted text-[13px] mb-3">
        Chọn mô hình, nhập ngân sách dự kiến. Chúng tôi gợi ý phần còn lại.
      </p>
      ...
      {/* Budget & Rent */}
      <SectionCard title="Ngân sách & Chi phí thuê">
        <div className="grid grid-cols-2 gap-3.5 max-md:grid-cols-1">
          <div>
            <label className="block font-semibold text-[13px] mb-1.5 text-text font-[family-name:var(--font-heading)]">
              Tổng ngân sách đầu tư dự kiến
              <Tooltip text="Tổng số tiền bạn có thể bỏ ra để mở quán — bao gồm tiền thuê, sửa sang, thiết bị, vốn xoay..." />
            </label>
            <VNDInput value={budget} onChange={setBudget} placeholder="VD: 500.000.000" />
            {model && (
              <div className="text-xs text-text-muted mt-1.5 leading-relaxed">
                {model.name} thường đầu tư: {model.investRange}
              </div>
            )}
          </div>
          <div>
            ...
            <VNDInput value={rent} onChange={setRent} placeholder="VD: 25.000.000" />
            {model ? (
              <div className="text-xs text-text-muted mt-1.5 leading-relaxed">
                {model.name} thường thuê: {formatVND(model.defaults.rent * 0.6)} - {formatVND(model.defaults.rent * 1.5)}/tháng
              </div>
            ) : (
              <div className="text-xs text-text-muted mt-1.5">Chọn mô hình để xem gợi ý</div>
            )}
          </div>
        </div>
      </SectionCard>

      <NavButtons onBack={() => useWizardStore.getState().setStep(0)} backLabel="Trang chủ" onNext={handleNext} />
    </div>
  );
}
